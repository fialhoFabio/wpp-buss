            ...
            <div className='space-y-3'>
              ...
              <div className='grid gap-3 sm:grid-cols-2'>
                {account.phoneNumbers.map((number) => {
                  ...
                  const needsRegistration = !needsVerification && (!number.quality_rating || number.quality_rating === 'UNKNOWN');
                  const isActive = !needsVerification && !needsRegistration;

                  return (
                    <div
                      key={number.id}
                      className={`flex flex-col rounded-lg border shadow-sm transition-shadow hover:shadow-md overflow-hidden
                        ${needsVerification ? 'border-yellow-300 bg-yellow-50/40' :
                          needsRegistration ? 'border-blue-300 bg-blue-50/40' :
                          'border-gray-200 bg-white'}`}
                    >
                      {/* Card body */}
                      <div className='flex items-start gap-3 p-3'>
                        <div className={`flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full
                          ${needsVerification ? 'bg-yellow-100 text-yellow-600' :
                            needsRegistration ? 'bg-blue-100 text-blue-600' :
                        ...
                        <div className='flex-1 min-w-0'>
                          <div className='flex items-center justify-between gap-2'>
                            <p className='text-sm font-semibold text-gray-900 truncate'>{number.display_phone_number}</p>
                            {isActive && (
                      ...
                      {/* Action banner */}
                      {needsVerification && (
                        <div className='flex items-center justify-between gap-3 border-t border-yellow-200 bg-yellow-100/60 px-3 py-2'>
                          <p className='text-[11px] font-medium text-yellow-800'>
                            Número precisa ser verificado via SMS ou ligação
                          </p>
                          <button
                            type='button'
                            onClick={() => setVerifyingNumber(number)}
                            className='shrink-0 inline-flex items-center gap-1 rounded-md bg-yellow-600 px-2.5 py-1 text-[11px] font-semibold text-white shadow-sm hover:bg-yellow-700'
                          >
                      ...
                      {needsRegistration && (
                        <div className='flex items-center justify-between gap-3 border-t border-blue-200 bg-blue-100/60 px-3 py-2'>
                          <p className='text-[11px] font-medium text-blue-800'>
                            Verificado — registre o número na API do Meta
                          </p>
                          <button
                            type='button'
                            onClick={() => setRegisteringNumber(number)}
                            className='shrink-0 inline-flex items-center gap-1 rounded-md bg-blue-600 px-2.5 py-1 text-[11px] font-semibold text-white shadow-sm hover:bg-blue-700'
                          >
                            <Icons.CheckCircle className='h-3 w-3' />
                            Registrar
                          </button>
