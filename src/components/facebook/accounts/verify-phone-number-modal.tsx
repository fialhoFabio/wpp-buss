...
import { requestWabaPhoneNumberCode, verifyWabaPhoneNumberCode } from 'lib/facebook';
    ...
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/50' onClick={onClose}>
      <div className='w-full max-w-md rounded-xl bg-white p-6 shadow-xl' onClick={e => e.stopPropagation()}>
        ...
        <div className='mb-1 flex items-center justify-between'>
          <h2 className='text-base font-semibold text-gray-900'>Verificar número</h2>
          ...
          {(['request', 'verify'] as Step[]).map((s, i) => (
            <div key={s} className='flex items-center gap-2'>
              <div className={`flex h-5 w-5 items-center justify-center rounded-full text-[10px] font-bold
                ${step === s ? 'bg-blue-600 text-white' : step === 'verify' && s === 'request' ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-500'}`}>
                {step === 'verify' && s === 'request' ? <Icons.Check className='h-3 w-3' /> : i + 1}
              </div>
              <span className={`text-[11px] font-medium ${step === s ? 'text-blue-600' : 'text-gray-400'}`}>
                {s === 'request' ? 'Enviar código' : 'Confirmar código'}
              </span>
